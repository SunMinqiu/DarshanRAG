...
Usage:
    # Using config_paths module (recommended)
    python build_darshan_kg.py \\
        --input_path $(python -c "from config_paths import get_parsed_log_path; print(get_parsed_log_path('2025-1-1'))") \\
        --output_path $(python -c "from config_paths import get_kg_path; print(get_kg_path('kg', '2025-1-1'))")

    # Or with hardcoded paths (not recommended)
...
# Add parent directory to path for config_paths import
...
try:
    ...
    print("⚠️  Warning: config_paths module not found. Using raw paths.")
...
def main():
    # Set default paths from config if available
    if CONFIG_AVAILABLE:
        default_input = str(PATHS['parsed_logs'])
        default_output = str(PATHS['kg_root'] / 'darshan_kg.json')
        epilog_text = """
Examples:
  # Using config_paths (recommended - paths auto-configured)
  python build_darshan_kg.py  # Uses default paths from config

  # Specify date for parsed logs
  python build_darshan_kg.py --date 2025-1-1
  ...
  python build_darshan_kg.py --input_path /path/to/logs/ --output_path graph.json
        ...
        default_input = None
        default_output = 'darshan_kg.json'
        epilog_text = """
Examples:
  ...
  python build_darshan_kg.py --input_path /path/to/logs/ --output_path graph.json
    ...
    parser.add_argument(
        ...
        help='Path to Darshan log file(s). Can be a single .txt file or folder (will traverse recursively).'
    )
    ...
    parser.add_argument(
        ...
        help='Output path for the knowledge graph JSON file'
    )
    ...
    if CONFIG_AVAILABLE:
        parser.add_argument(
            '--date',
            type=str,
            help='Date string for parsed logs (e.g., 2025-1-1). Overrides --input_path.'
        )

    args = parser.parse_args()

    # Handle date parameter if config is available
    if CONFIG_AVAILABLE and args.date:
        args.input_path = str(get_parsed_log_path(args.date))
        if not args.output_path or args.output_path == default_output:
            args.output_path = str(get_kg_path('kg', args.date))
