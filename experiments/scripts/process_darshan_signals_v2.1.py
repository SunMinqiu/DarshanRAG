#!/usr/bin/env python3
"""
Darshan Log Signal Extraction Tool v2.1
- Removed "Preserved" text
- Fixed parsing to capture all metrics with file metadata
- Added NA reason annotations
- Added HEATMAP-specific signals
"""
...
class DarshanLogProcessor:
    ...
    def parse_log_file(self, input_path):
        ...
        with open(input_path, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            for line in f:
                ...
                # Store file metadata if available (columns 6-8)
                if len(parts) >= 8:
                    file_name = parts[5]
                    mount_pt = parts[6]
                    fs_type = parts[7]
    ...
    def compute_heatmap_signals(self, module, record_id, record):
        ...
        N = max(len(read_bins), len(write_bins))
        if N == 0:
            return

        # Pad with zeros if needed
        while len(read_bins) < N:
            read_bins.append(0)
        while len(write_bins) < N:
            write_bins.append(0)
        ...
        # 3. active_bins (number of bins with activity > 0)
        ...
        # 5. activity_span (from first to last active bin)
        first_active = -1
        last_active = -1
        ...
        # 6. peak_activity_bin
        if activity_bins:
            signals['peak_activity_bin'] = max(activity_bins)
        else:
            signals['peak_activity_bin'] = 0
        ...
        # 9. top1_share (largest bin's share of total activity)
    ...
    def compute_record_signals(self, module, record_id, record):
        ...
        if 'HEATMAP' in module:
            self.compute_heatmap_signals(module, record_id, record)
            return
    ...
    def write_signals_output(self, output_path):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write("\n")
            f.write("# ============================================================\n")
            f.write("# SIGNAL EXTRACTION OUTPUT v2.1\n")
                ...
                for record_id in sorted(self.modules[module].keys()):
                    ...
                    if 'HEATMAP' in module:
                        ...
                        for key in ['total_read_events', 'total_write_events', 'active_bins',
                                   'active_time', 'activity_span', 'peak_activity_bin',
...
def process_directory(input_dir, output_dir):
    ...
    for txt_file in txt_files:
        ...
        output_file = output_path / rel_path.parent / f"{rel_path.stem}_signals_v2.1.txt"
...
def main():
    parser = argparse.ArgumentParser(
        description='Extract Darshan log signals v2.1',
    ...
    if input_path.is_file():
        if args.output:
            ...
            output_file = input_path.parent / f"{input_path.stem}_signals_v2.1.txt"
        ...
        if args.output:
            ...
            output_dir = input_path.parent / f"{input_path.name}_signals_v2.1"
