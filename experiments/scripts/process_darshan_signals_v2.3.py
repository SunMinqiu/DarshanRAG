#!/usr/bin/env python3
"""
Darshan Log Signal Extraction Tool v2.3
- Output format matches original Darshan log format
- Added: Signal description comments for each section
- Added: Table headers: #<module> <rank> <record id> <counter> <value> <file name> <mount pt> <fs type>
- All signals include formulas in comments
- Maintains v2.2 logic: (module, rank, record_id) uniqueness, HEATMAP fixes
"""
...
class DarshanLogProcessor:
    ...
    def get_signal_descriptions(self, module):
        ...
        if 'POSIX' in module or 'STDIO' in module or 'MPI-IO' in module:
            return """# description of derived signals:
...
#   SIGNAL_WRITE_IOPS: write operations per second = writes / write_time
...
#   SIGNAL_META_OPS: total metadata operations = opens + stats + seeks + fsyncs + fdsyncs (POSIX only)
#   SIGNAL_META_INTENSITY: metadata intensity = meta_ops / (reads + writes) (POSIX only)
#   SIGNAL_META_FRACTION: metadata time fraction = meta_time / total_time (POSIX only)
#   SIGNAL_UNALIGNED_READ_RATIO: unaligned read ratio = not_aligned_reads / reads (POSIX only)
#   SIGNAL_UNALIGNED_WRITE_RATIO: unaligned write ratio = not_aligned_writes / writes (POSIX only)
#   SIGNAL_SMALL_READ_RATIO: small read ratio (< 10KB) = (size_0-100 + size_100-1K + size_1K-10K) / reads (POSIX only)
#   SIGNAL_SMALL_WRITE_RATIO: small write ratio (< 10KB) = (size_0-100 + size_100-1K + size_1K-10K) / writes (POSIX only)
#   SIGNAL_REUSE_PROXY: data reuse proxy = bytes_read / (MAX_BYTE_READ + 1) (POSIX only)
#   SIGNAL_RANK_IMBALANCE_RATIO: rank imbalance ratio = slowest_rank_bytes / fastest_rank_bytes (shared files only, rank=-1)
#   SIGNAL_BW_VARIANCE_PROXY: bandwidth variance proxy = variance_rank_bytes (shared files only, rank=-1)
...
def process_directory(input_dir, output_dir):
    ...
    for txt_file in txt_files:
        ...
        output_file = output_path / rel_path.parent / f"{rel_path.stem}_signals_v2.3.txt"
...
def main():
    parser = argparse.ArgumentParser(
        description='Extract Darshan log signals v2.3',
    ...
    if input_path.is_file():
        if args.output:
            ...
            output_file = input_path.parent / f"{input_path.stem}_signals_v2.3.txt"
        ...
        if args.output:
            ...
            output_dir = input_path.parent / f"{input_path.name}_signals_v2.3"
